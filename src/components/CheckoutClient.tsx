    ...
    return (
      <div>
        ...
        <input
          ...
          onChange={(e) => { onChange({ ...details, [id]: e.target.value }); if (errors[id]) setErrors((prev) => ({ ...prev, [id]: undefined })); }}
    ...
    <div>
      ...
      <div className="flex flex-col gap-6 mb-10">
        ...
        <div>
          ...
          <textarea
            id="notes" value={details.notes}
            onChange={(e) => onChange({ ...details, notes: e.target.value })}
            placeholder="any other details for your order" rows={3}
      ...
      <div className="flex items-center gap-6">
        <button onClick={onBack} className="font-inter text-xs tracking-widest uppercase text-ink/40 hover:text-burgundy transition-colors">← back</button>
        <button onClick={handleNext} className="bg-burgundy text-cream px-10 py-4 font-inter text-xs tracking-widest uppercase hover:bg-ink transition-colors">continue →</button>
      </div>
    </div>
  );
}
...
function PaymentStep({
  details, errors, onChange, onBack,
}: {
  ...
  function paymentField(id: keyof PaymentDetails, label: string, placeholder: string, maxLength: number) {
    return (
      <div>
...
function OrderSummaryPanel({
  items, subtotal, isOnPaymentStep, onPlaceOrder, submitting,
}: {
  items: CartItem[];
  subtotal: number;
  ...
  return (
    <div className="lg:sticky lg:top-32 self-start border border-ink/10 p-8">
      ...
      <div className="border-t border-ink/10">
        {items.map((item) => (
          <div key={item.productId} className="flex justify-between gap-4 py-4 border-b border-ink/10">
            ...
            <p className="font-inter text-sm text-ink shrink-0">${(item.priceValue * item.quantity).toFixed(2)}</p>
          </div>
        ))}
      </div>
      ...
      <button
        ...
        disabled={!isOnPaymentStep || submitting}
        className={`w-full py-4 font-inter text-xs tracking-widest uppercase transition-colors ${
          isOnPaymentStep && !submitting ? "bg-burgundy text-cream hover:bg-ink" : "bg-ink/10 text-ink/30 cursor-not-allowed"
        }`}
      >
        {submitting ? "placing order..." : "place order"}
      </button>
      {!isOnPaymentStep && (
        <p className="font-inter text-xs text-ink/30 text-center mt-3">complete all steps to place your order</p>
      )}
    </div>
  );
}
...
export default function CheckoutClient({ pickupDate, pickupTime }: Props) {
  ...
  const [customerDetails, setCustomerDetails] = useState<CustomerDetails>({ name: "", email: "", phone: "", notes: "" });
  ...
  return (
    <main className="px-8 md:px-20 pt-40 pb-20 min-h-screen">
      ...
      {submitError && (
        <p className="font-inter text-xs text-burgundy mb-6">{submitError}</p>
      )}

      <div className="grid grid-cols-1 lg:grid-cols-[1fr_380px] gap-16">
        <div>
          {step === 1 && (
            <OrderSummaryStep items={items} subtotal={subtotal} pickupDate={pickupDate} pickupTime={pickupTime} onNext={() => setStep(2)} />
          )}
          {step === 2 && (
            <DetailsStep details={customerDetails} onChange={setCustomerDetails} onBack={() => setStep(1)} onNext={() => setStep(3)} />
          )}
          {step === 3 && (
            <PaymentStep details={paymentDetails} errors={paymentErrors} onChange={setPaymentDetails} onBack={() => setStep(2)} />
          )}
        </div>
